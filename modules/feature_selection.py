...
class SafeSelectByShuffling(SelectByShuffling):
    ...
    def transform(self, X):
        ...
        if n_features_left >= self.min_features:
            ...
            features, drifts = zip(*self.performance_drifts_.items())                     # split a dictionary into keys and values
            features = np.array(features)[np.argsort(drifts)[::-1]]                       # sort feature names in descending order of metric drifts
